...
- Fuzzy-matches task companies to leads (handles diacritics, abbreviations, spacing differences)
- Detects pipeline stage from task subject patterns (Intro Meeting → Demo → POC → Commercial)
- Flags leads needing attention (stale, no activity, negative outcomes, open tasks)
...
sales-dashboard/
  ...
  db.js             — SQLite init + schema (3 tables: leads, tasks, lead_task_links)
  matching.js       — Company name normalization + fuzzy matching
  sales-stage.js    — Pipeline stage detection from task subjects
  import.js         — Excel parsing + upsert into SQLite
  server.js         — Express API (7 endpoints) + static file serving
...
- Attention alerts (stale, open tasks, negative outcomes, stuck in early stage)
- Full activity timeline with expandable comments (individual or expand-all)
- Lemlist tasks hidden by default (toggle to show)
- Linked companies with match type and confidence
...
| GET | `/api/leads/:id/timeline` | Task timeline + stage details for one lead |
| GET | `/api/summary` | Dashboard stats (totals, by-stage, by-owner, needing attention) |
| GET | `/api/unmatched` | Task companies with no matching lead |
...
- When multiple leads share the same company name, only the first gets linked to tasks
